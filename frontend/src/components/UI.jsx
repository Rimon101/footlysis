...
export function StatCard({ label, value, sub, trend, color = 'white', icon: Icon, link }) {
  ...
  return (
    <Wrapper {...wrapperProps} className={`metric-card${link ? ' cursor-pointer hover:border-brand-500/40 hover:bg-white/[0.04] transition-all' : ''}`}>
      ...
      <div className={`text-2xl font-bold mt-1 ${color === 'green' ? 'text-emerald-400' : color === 'red' ? 'text-red-400' : color === 'yellow' ? 'text-amber-400' : 'text-white'}`}>
        {value ?? '—'}
      </div>
      {(sub || trend !== undefined) && (
...
export function Badge({ children, variant = 'default' }) {
  const variants = {
    ...
    green: 'bg-emerald-500/20 text-emerald-300 border border-emerald-400/35',
    red: 'bg-red-500/20 text-red-400 border border-red-500/30',
    yellow: 'bg-amber-500/20 text-amber-300 border border-amber-400/35',
    blue: 'bg-indigo-500/20 text-indigo-300 border border-indigo-400/35',
  }
  return (
